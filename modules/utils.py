...
def mixed_checkpoint(func, inputs: dict, params, flag):
    ...
    reduced memory at the expense of extra compute in the backward pass. This differs from the
    original checkpoint function borrowed from
    https://github.com/openai/guided-diffusion/blob/0ba878e517b276c45d1195eb29f6f5f72659a05b/guided_diffusion/nn.py in that
...
class MixedCheckpointFunction(torch.autograd.Function):
    ...
    def forward(
        ...
        *args,
        ...
        ctx.input_tensors = {
            key: val for (key, val) in zip(tensor_keys, list(args[: ctx.end_tensors]), strict=False)
        }
        ctx.input_non_tensors = {
            key: val
            for (key, val) in zip(
                non_tensor_keys, list(args[ctx.end_tensors : ctx.end_non_tensors]), strict=False
            )
        }
    ...
    def backward(ctx, *output_grads):
        # additional_args = {key: ctx.input_tensors[key]
        #  for key in ctx.input_tensors if not isinstance(ctx.input_tensors[key],torch.Tensor)}
        ctx.input_tensors = {
