...
#SBATCH --output=logs/i2v_ltx_%j.out
#SBATCH --error=logs/i2v_ltx_%j.err

set -euo pipefail
cd "$(dirname "$0")"
mkdir -p logs

echo "=== I2V LTX-2B Experiment ==="
echo "Node: $(hostname)"
...
accelerate launch --num_processes 8 --mixed_precision bf16 main.py \
    -c configs/flowception/i2v/openvid_ltx_i2v.yaml \
    -t -n ltx_i2v_10k \
    --append \
