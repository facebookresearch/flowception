...
def write_frames_to_video(frames, outpath, fps=24,):
...
def write_frames_ffmpeg(frames, outpath, fps=24):
    ...
    iio.imwrite(
        outpath, frames, plugin="FFMPEG",
        fps=fps, codec="libx264", pixelformat="yuv420p",  # widely compatible
...
def frames_to_gif(frames_t, fps=24, save_path=None):
    ...
    frames = [(255*(frames_t[:, i].clip(0,1)).transpose(1,2,0)).astype(np.uint8) for i in range(frames_t.shape[1])]
